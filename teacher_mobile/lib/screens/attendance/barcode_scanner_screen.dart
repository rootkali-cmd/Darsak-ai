...
class _BarcodeScannerScreenState extends State<BarcodeScannerScreen> {
  ...
  void _onDetect(BarcodeCapture capture) {
    ...
    final student = _parseStudentBarcode(raw);
    if (student != null) {
      _processScan(student);
    }
  }

  ScannedStudent? _parseStudentBarcode(String raw) {
    ...
    // Fallback: any barcode code up to 30 chars alphanumeric
    if (clean.length <= 30) {
      return ScannedStudent(
        id: clean,
        code: clean,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: notification.color.withValues(alpha: 0.3), width: 1.5),
        boxShadow: [
          BoxShadow(

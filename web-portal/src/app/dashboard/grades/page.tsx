    ...
    <div className="space-y-6">
      <Section>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[var(--text-muted)] mt-1">إدارة درجات الامتحانات</p>
          </div>
          <div className="flex gap-3 w-full sm:w-auto">
            <NeonButton variant="outline" className="flex-1 sm:flex-none"><Upload className="w-5 h-5" /> رفع CSV</NeonButton>
            <NeonButton onClick={() => setShowAddModal(true)} className="flex-1 sm:flex-none"><Plus className="w-5 h-5" /> إضافة درجة</NeonButton>
          </div>
        </div>
      </Section>
      ...
      {gradeStats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Section delay={0.1}><GlassCard className="text-center"><div className="text-4xl font-black "><CountUp end={Math.round(gradeStats.average || 0)} suffix="%" /></div><p className="text-[var(--text-muted)] text-sm mt-1">المتوسط</p></GlassCard></Section>
          <Section delay={0.2}><GlassCard className="text-center border-green-500/20"><div className="text-4xl font-black text-green-500"><CountUp end={Math.round(gradeStats.highest || 0)} suffix="%" /></div><p className="text-[var(--text-muted)] text-sm mt-1">أعلى درجة</p></GlassCard></Section>
          <Section delay={0.3}><GlassCard className="text-center border-red-500/20"><div className="text-4xl font-black text-red-500"><CountUp end={Math.round(gradeStats.lowest || 0)} suffix="%" /></div><p className="text-[var(--text-muted)] text-sm mt-1">أقل درجة</p></GlassCard></Section>
          <Section delay={0.4}><GlassCard className="text-center"><div className="text-4xl font-black"><CountUp end={gradeStats.total || 0} /></div><p className="text-[var(--text-muted)] text-sm mt-1">إجمالي الدرجات</p></GlassCard></Section>
        </div>
      )}
        ...
        <GlassCard>
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
                    ...
                    return (
                      <motion.tr key={grade.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: index * 0.05 }} className="border-b border-black/5 hover:bg-[rgba(0,0,0,0.02)] transition-colors">
                        ...
                        <td className="py-3 px-4 text-center">
                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="p-2 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20" onClick={() => { if (confirm('هل أنت متأكد؟')) deleteMutation.mutate(grade.id) }}>
              ...
              {(!grades || grades.length === 0) && <div className="text-center py-12 text-[var(--text-muted)]">لا توجد درجات</div>}
            </div>
          )}
        </GlassCard>
      </Section>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
            <motion.div initial={{ scale: 0.8, opacity: 0, y: 50, filter: 'blur(10px)' }} animate={{ scale: 1, opacity: 1, y: 0, filter: 'blur(0px)' }} exit={{ scale: 0.8, opacity: 0, y: 50, filter: 'blur(10px)' }} transition={{ type: 'spring', damping: 25 }} className="card rounded-2xl p-6 w-full max-w-md border border-black/10 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate({ ...formData, score: parseFloat(formData.score), max_score: parseFloat(formData.max_score) }) }} className="space-y-4">
                ...
                <input placeholder={searchMode === 'code' ? 'كود الطالب (مثال: STU-A1B)' : 'اسم الطالب'} value={searchValue} onChange={(e) => { setSearchValue(e.target.value); setFormData({ ...formData, student_id: '' }) }} className="input w-full px-4 py-3 rounded-xl text-[var(--text)] placeholder:text-[var(--text-muted)]" dir="ltr" />
                ...
                {searchValue && filteredStudents && filteredStudents.length === 0 && (
                  <p className="text-sm text-[var(--text-muted)]">لم يتم العثور على طالب</p>
                )}
                <input placeholder="اسم الامتحان" value={formData.exam_name} onChange={(e) => setFormData({ ...formData, exam_name: e.target.value })} className="input w-full px-4 py-3 rounded-xl text-[var(--text)] placeholder:text-[var(--text-muted)]" required />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input type="number" placeholder="الدرجة" value={formData.score} onChange={(e) => setFormData({ ...formData, score: e.target.value })} className="input w-full px-4 py-3 rounded-xl text-[var(--text)] placeholder:text-[var(--text-muted)]" required min="0" />
                  <input type="number" placeholder="من" value={formData.max_score} onChange={(e) => setFormData({ ...formData, max_score: e.target.value })} className="input w-full px-4 py-3 rounded-xl text-[var(--text)] placeholder:text-[var(--text-muted)]" required min="1" />
                </div>
                <textarea placeholder="ملاحظات" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="input w-full px-4 py-3 rounded-xl text-[var(--text)] placeholder:text-[var(--text-muted)]" rows={3} />
